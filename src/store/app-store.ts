...
import { BOARD_COLORS } from "@/src/constants";
import { uid } from "@/src/lib/utils";
import { dayKey, isDoneThisPeriod, periodKey } from "@/src/lib/routines";
...
export const useAppStore = create<AppStore>()(
  persist(
    (set) => {
      // Reusamos el reducer: cada acción es un dispatch tipado sobre el store.
      const dispatch = (action: Action) => set((s) => reducer(s, action));
    ...
    },
    {
      ...
      partialize: (s) => ({ boards: s.boards, routines: s.routines }),
    },
  ),
);
  ...
  return [
    {
      ...
      tasks: [
        mk("Prepare Q3 presentation", "high", "in_progress", {
          due: 2,
          desc: "Include sales metrics.",
        }),
        mk("Reply to pending emails", "medium", "done", {
          created: 2,
          completed: 1,
        }),
        mk("Update project documentation", "low", "todo", { due: 7 }),
        mk("Review the team's pull requests", "medium", "done", {
          created: 4,
          completed: 2,
        }),
        mk("Plan next sprint", "high", "done", { created: 35, completed: 33 }),
        mk("Onboard new teammate", "medium", "done", {
          created: 70,
          completed: 66,
        }),
        mk("Refactor billing module", "high", "done", {
          created: 100,
          completed: 95,
        }),
      ],
    },
    {
      ...
      tasks: [
        ...
        mk("Renew gym membership", "low", "done", {
          created: 45,
          completed: 44,
        }),
        mk("Plan summer trip", "medium", "done", {
          created: 80,
          completed: 72,
        }),
      ],
    },
    {
      ...
      tasks: [
        mk("Finish the React course", "medium", "in_progress", {
          desc: "Advanced hooks module.",
        }),
        mk("Read a TypeScript chapter", "low", "done", {
          created: 3,
          completed: 1,
        }),
        mk("Build a project with Tailwind", "low", "todo"),
        mk("Complete algorithms practice", "medium", "done", {
          created: 50,
          completed: 47,
        }),
        mk("Watch system design talk", "low", "done", {
          created: 88,
          completed: 85,
        }),
      ],
    },
    {
      ...
      tasks: [
        ...
        mk("Morning run streak", "high", "done", {
          created: 20,
          completed: 18,
        }),
      ],
